...
//  ForDesign
//
//  Created by GIORGI PILISSHVILI on 16.09.22.
//
...
class NewChatViewModel: ObservableObject {
    ...
    func getUserChats() {
        ...
        firebaseManager.dbUsers.observeSingleEvent(of: .value) { snapshot in
            
            for dataSnapshot in snapshot.children.allObjects as! [DataSnapshot] {
                
                let currentUser = dataSnapshot.decode(class: User.self)
                guard let currentUser = currentUser else { return }
                
                if currentUser.id != FirebaseManager.currentUser?.uid ?? "" {
